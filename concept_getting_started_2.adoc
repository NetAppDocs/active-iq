...
| Support Renewals | Supported on all platforms | Supported on all platforms | Supported on all platforms

| Storage Expansion | Supported only on ONTAP | Supported only on ONTAP | Supported only on ONTAP

| Inventory  | Supported on all platforms | Supported only on ONTAP | Supported only on ONTAP

| Upgrades | Supported only on ONTAP | Supported only on ONTAP | Supported only on ONTAP

|===

NOTE: All platforms means ONTAP, E-Series, StorageGRID, ONTAP Select, and Cloud Volumes ONTAP.

// 29-JULY-2026 remove data cluster from ONTAP
