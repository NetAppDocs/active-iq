...
= Generate an ONTAP upgrade plan for single and multiple clusters in Digital Advisor
:toclevels: 1
:hardbreaks:
:nofooter:
:icons: font
...
Before generating an upgrade plan, you should:

* Ensure that AutoSupport (ASUP) is enabled and configured correctly, and all required sections are included. For more information, see link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-KBs/ONTAP_AutoSupport_Resolution_Guide[ONTAP AutoSupport Resolution Guide].
* Verify the AutoSupport's cluster members match the SAP cluster configuration. For more information, link:https://kb.netapp.com/Support/NSS/Support_Site/How_to_open_a_non-technical_case[open a non-technical case].
...
.Single cluster
--
. On the dashboard, select the number of clusters in the *Upgrade Advisor* widget.
+
...
. Select a cluster for an upgrade. 
  The *Target OS Version* column displays the recommended target OS version. You can select the image:edit_icon.png[Image of the edit button] icon to select another target OS version of a cluster.
+ 
image:r_ua_select_target_OS_version_single_cluster.png[An image that shows how to select target version.] 
+
 * You can select the number of nodes in the *Nodes* column to see the node summary of a cluster.
 * You can select *Resolve Issues* from the *Recommended Action* column to fix the issues with an ineligible cluster to make it eligible for an upgrade.
 +
image:r_ua_resolve_issue.png[An image that shows the type of issue and required resolution to fix it.]

. Select *Generate Upgrade Plan*.
 ...
 * You can select the image:ua_export_icon.png[export button] icon to export the risk summary to an Excel sheet.
 
. Select *Generate Upgrade Plan* from the *Upgrade Recommendation* page.
...
+
TIP: You can select *View Upgrade Advisor Reports* to go to the *Reports* page.
 +
image:r_ua_view_reports.png[A screenshot that shows the position where View Upgrade Advisor is located for selecting. ]
...
+
 * You can select the number of nodes in the *Nodes* column to see the node summary of a cluster.
 * You can select *Resolve Issues* from the *Recommended Action* column to fix the issues with ineligible clusters to make them eligible for an upgrade.
 +
image:r_ua_resolve_issue.png[An image that shows the type of issue and required resolution to fix it.]
