...
. Select link:https://mysupport.netapp.com/site/global/dashboard[here^] to open the NetApp Support site.
. From the top menu, select *My Cases*.
. Select *Create a Case*.
. Select a category, provide the required details, and select *New Search* to search for a system from your inventory. 
...
. Enter the problem description in the Problem Description field and select *Proceed*.
. Provide additional details about the problem.
. Select *Select Files* to upload files or images that may help diagnose the problem.
. Select *Proceed*.
...
. Select link:https://mysupport.netapp.com/site/help[here^] to open the “Non-Technical Feedback Tool" on the NetApp Support site.
...
. Select *Storage Efficiency* from the left navigation.
...
Yes, Digital Advisor displays ratios for all ONTAP systems.

*What is the 'With snapshots copies' toggle on the efficiency dashboard?*

The Digital Advisor Efficiency Dashboard calculates the data reduction ratio at multiple levels, including customer, site, group, cluster, and node. If you turn on the *With snapshot copies* toggle, the data reduction ratio calculation includes the following storage efficiency technologies:
...
By default, the data reduction ratio calculation does not include snapshot storage efficiency technology.
...
Customer-level storage efficiency dashboard provides the data reduction ratio with and without snapshot copies for AFF and non-AFF systems and are combined across the customer installed base. The required parameters for the following calculations are taken from ONTAP AutoSupport:
...
If you are unable to view systems for any other reasons, have queries, or wish to request for access, link:https://mysupport.netapp.com/site/help[create a non-technical case^]. To learn more about non-technical case, select <<Non-technical case,here.>>
...
** Displays the logical size used in the aggregate
** The aggregate includes volumes, clones, and snapshot copies.
** The logical size is computed based on physical usage (real writes) and savings obtained in the aggregate.
