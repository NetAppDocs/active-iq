...
By default, the Digital Advisor Efficiency Dashboard calculates the efficiency ratio at multiple levels, including customer, site, group, cluster, and node. This ratio includes the following storage efficiency technologies:
...
If you turn on the *With Snapshots copies* toggle, the efficiency ratio calculation also includes the Snapshots storage efficiency technology, and the ratio is calculated at the same levels, customer, site, group, cluster, and node.
