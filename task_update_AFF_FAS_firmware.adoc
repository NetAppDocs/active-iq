...
.Steps
. Select any wellness widget on the dashboard or select *View All Actions* to view a list of all the actions and risks.
. Select *Firmware Upgrade* to view all firmware upgrade risks.
. Select *Update AFF and FAS Firmware* to view all available update packages or select image:update_ansible.png[Update Ansible] next to each risk to update the package specific to that risk.
. Select *Download* to download the zip files and update your storage system.
+
