...
You can use the StorageGRID Dashboard for viewing information at the Watchlist, Customer, and Group, Site level.
...
* *Planning widget:*
** *Capacity Addition:* For any GRID sites that are exceeding the threshold of 70% of existing capacity, you’ll be notified. It also provides a forecast for the next 1, 3, 6, & 9 months if the capacity threshold is likely to exceed 70%.
** *Renewals:* For any StorageGRID systems for which the license contract that is either expired or nearing expiration in the next 6 months, you’ll be notified. You can select one or more systems to raise a request to the NetApp support team for renewal.
...
* *GRID Viewer:* Under the configuration section, you can view the GRID configuration in detail by clicking the *GRID Viewer* link.
* *GRID summary:* Contains basic information, such as License Type, License Capacity, # of installed nodes, Support Term (Date of termination of license contract), Primary Admin Node, and Primary Site of the Primary Admin Node
* *Site details:* Contains site name and the number of storage nodes tagged under the corresponding site. In this release, you can view the list of node names upon clicking the hyperlink available for viewing storage nodes for the corresponding site.
* *Capacity Details tab:* Provides the capacity details configured for the GRID and respective sites under the selected GRID.
* *GRID Viewer (Configuration Details) as Download:* You can download the site details and capacity details for the selected StorageGRID by clicking on the *Download* button.
