...
=== Upgrade Plan
The upgrade plan has been optimized to remove redundant and common steps across all nodes in a cluster, remove outdated upgrade rules from the rule inventory, and simplify the backout plan. To view the improved plan, generate the upgrade plan using the steps mentioned in link:https://docs.netapp.com/us-en/active-iq/task_view_upgrade.html[Upgrade the ONTAP version using upgrade plan].
