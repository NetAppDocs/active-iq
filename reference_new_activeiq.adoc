...
=== Upgrade Plan
An upgrade plan that is used to upgrade to a newer version of ONTAP has been improved. Redundant and common steps across all cluster nodes are optimized, and outdated rules are removed from the rule inventory. To view the improved plan, generate the upgrade plan using the steps mentioned in link:https://docs.netapp.com/us-en/active-iq/task_view_upgrade.html[Upgrade the ONTAP version using upgrade plan].
