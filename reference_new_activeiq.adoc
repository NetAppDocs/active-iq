...
=== Upgrade Advisor
You can access the Upgrade Advisor page for your default Watchlist or Dashboard using the link:https://activeiq.netapp.com/redirect/upgrade-advisor[Upgrade Advisor^] link. If you do not have a default Watchlist or Dashboard, clicking the link will redirect you to the Search page.


=== Upgrade Plan
The upgrade plan has been optimized to remove redundant upgrade steps, modify information and warnings, and simplify the backout plan. The common steps across all nodes in a cluster are consolidated and available under the General Information section. To view the improved plan, generate the upgrade plan using the steps mentioned in link:https://docs.netapp.com/us-en/active-iq/task_view_upgrade.html[Upgrade the ONTAP version using upgrade plan].
